...
export default function Details() {
  ...
  useEffect(() => {
    ...
    if (cropItem) {
      const categoryItems =
        cropItem && cropItem?.categories.length
          ? categoryHook.getNamesByIds(cropItem?.categories)
          : [];
      setCrop(cropItem);
          ...
          >
            ...
            >
              <View style={tailwind`flex flex-row justify-center items-center`}>
                <Text
                  style={tailwind`flex-1 text-[2rem] font-extrabold text-teal-900`}
                >
                  {crop.title}
                </Text>
              ...
              <Text
                style={tailwind`text-sm md:text-lg font-normal text-slate-500`}
              >
